...
#    bin/ffmpeg -y -progress url -nostdin -hide_banner -loglevel error -loop 1 -i "$1" -t 00:00:05 -filter_complex "[0:v]fps=fps=60,scale=$6,setsar=1/1,setdar=$7" -c:v h264_nvenc -pix_fmt yuv420p -pix_fmt yuv420p -tune stillimage  "mp4/out.$COUNT.mp4"
    ...
    # echo -n "Files:"
    echo "Files:"
    echo "bin/ffmpeg_g \\" > ffmpeg_command.part.plus1
    echo "-filter_complex \" \\" > ffmpeg_command.part.plus2
    # echo -n "[v0a1]" > ffmpeg_command.part.plus3
    rm ffmpeg_command.part.plus3
    p="0"
...
#        echo p=$p i=$i n=$n
        ...
        TRANS3=$(echo "($TRANS-$TRANSTIME)"| bc -l);  
        TRANS3="${TRANS2%0000}"
...
###            if [ ! "$i" == "0" ]; then
#                echo "[v"$i"a]trim="$TRANSTIME":"$TRANS",setpts=PTS-STARTPTS[v"$i"A]; \\" >> ffmpeg_command.part.plus2
                echo "["$i":v]trim=0.0:"$TRANS",setpts=PTS-STARTPTS[v"$i"B]; \\" >> ffmpeg_command.part.plus2
#                echo "[v"$i"B]pipglshader=start="$TRANS":duration="$TRANSTIME":pip_duration="$TRANSTIME":ext_source="$NAME_AN":vxsource=gl_pip/pip_vertex.gl:sdsource=gl_pip/pip_shader.gl[out"$i"]; \\" >> ffmpeg_command.part.plus2
                echo "[v"$i"B]pipglshader=start=0.0:duration="$TRANS":pip_duration="$TRANS":ext_source="$NAME_AN":vxsource=gl_pip/pip_vertex2.gl:sdsource=gl_pip/pip_shader2.gl[out"$i"]; \\" >> ffmpeg_command.part.plus2
###            else 
#                echo "[v"$i"a]trim=0.0:"$TRANS"[v"$i"A]; \\" >> ffmpeg_command.part.plus2
###                echo "["$i":v]trim=0.0:"$TRANS",setpts=PTS-STARTPTS[out"$i"]; \\" >> ffmpeg_command.part.plus2
#                echo "["$i":v]trim=0.0:"$TRANS",setpts=PTS-STARTPTS[v"$i"B]; \\" >> ffmpeg_command.part.plus2
#                echo "[v"$i"B]pipglshader=start="$TRANS":duration="$TRANSTIME":pip_duration="$TRANSTIME":ext_source="$NAME_AN":vxsource=gl_pip/pip_vertex.gl:sdsource=gl_pip/pip_shader.gl[out"$i"]; \\" >> ffmpeg_command.part.plus2
#                echo "[v"$i"B]pipglshader=start=0.0:duration="$TRANS":pip_duration="$TRANS":ext_source="$NAME_AN":vxsource=gl_pip/pip_vertex2.gl:sdsource=gl_pip/pip_shader2.gl[out"$i"]; \\" >> ffmpeg_command.part.plus2
###            fi
#            echo "[v"$i"b]trim="$TRANS":"$LENGTH",setpts=PTS-STARTPTS[v"$i"B]; \\" >> ffmpeg_command.part.plus2
#            echo "[v"$i"b]trim=0.0:"$LENGTH",setpts=PTS-STARTPTS[v"$i"B]; \\" >> ffmpeg_command.part.plus2
            echo >> ffmpeg_command.part.plus2
#            echo -n "[v"$i"A]" >> ffmpeg_command.part.plus3
            echo -n "[out"$i"]" >> ffmpeg_command.part.plus3
#            add=$(echo "($add+$ONE)"| bc -l);  
...
###            fi
            ...
            echo "["$i":v]trim=0.0:"$LENGTH",setpts=PTS-STARTPTS[v"$i"B]; \\" >> ffmpeg_command.part.plus2
...
#  echo "-map \"[outa]\" \\" >> ffmpeg_command.part.plus3
  ...
  cat ffmpeg_command.part.plus1 > ffmpeg_command2.$DIR.sh
  cat ffmpeg_command.part.plus2 >> ffmpeg_command2.$DIR.sh
  cat ffmpeg_command.part.plus3 >> ffmpeg_command2.$DIR.sh

  chmod +x ffmpeg_command2.$DIR.sh
#  exit
}
...
# MAKE_FFMPEG "movie/simple.mp4"

#CORRECT="0"
MAKE_FFMPEG2 "movie/simple.mp4"
TEST_MP4 "movie/simple.*.mp4"
./ffmpeg_command2.movie.sh
ffmpeg -y -i output_movie.mp4 -i movie/simple.mp3 -t 00:03:17 -c copy movie_sound.mp4 

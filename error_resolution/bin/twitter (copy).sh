...
func_add () {
    ...
    bin/ffmpeg -y -progress url -nostdin -hide_banner -loglevel error -loop 1 -i "$1" -t 00:00:15.1 -filter_complex "[0:v]fps=fps=60,scale=$6,setsar=1/1" -c:v h264_nvenc -pix_fmt yuv420p -pix_fmt yuv420p -tune stillimage  "mp4/out.$COUNT.mp4"
...
TEST_MP4 () {
    printf "$fmt" w h sar dar duration filename
    for line in $1
    do
...
## MAIN "jpg/*.jpg"
## MAKE_FFMPEG "mp4/out.mp4"
## TEST_MP4 "mp4/*.mp4"
### #test_fadeglshader1
### 
### mkdir out
### ffmpeg -i mp4/out.01.mp4 -vsync drop -q:v 2 out/out.01.%d.jpg
